...
function LoginContent() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const r = await signIn.email({ email: form.email, password: form.password });
      if (r.error) {
        toast.error(r.error.message || "Invalid email or password.");
      } else {
    ...
    <div className="auth-wrap">
      <div className="auth-box fade-up">
        ...
        <div style={{ textAlign: "center", marginBottom: "1.75rem" }}>
          ...
          <h1 style={{ fontSize: "1.5rem", fontWeight: 700, color: "var(--c-text)", marginBottom: 4 }}>Welcome Back</h1>
          <p style={{ color: "var(--c-muted)", fontSize: ".85rem" }}>Sign in to continue reading</p>
        </div>
        ...
        <form onSubmit={submit} style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
          ...
          <button type="submit" disabled={loading} className="btn btn-fill"
            ...
            {loading
              ? <><svg className="spin" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 12a9 9 0 1 1-6.219-8.56"/></svg>Signing In…</>
              : "Sign In →"
            }
          </button>
        </form>
...
export default function LoginPage() {
  return (
    <Suspense fallback={<div className="auth-wrap"><span style={{ color: "var(--c-muted)" }}>Loading…</span></div>}>
      <LoginContent />
    </Suspense>
  );
}
