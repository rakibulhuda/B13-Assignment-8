    ...
    return (
        <div className="card fade-up" style={{ display: "flex", flexDirection: "column", animationDelay: `${index * 0.07}s` }}>
            ...
            <div style={{ padding: "1rem 1rem 1.1rem", flex: 1, display: "flex", flexDirection: "column" }}>
                ...
                {/* Title */}
                <h3 style={{ color: "var(--c-text)", fontSize: ".9rem", fontWeight: 700, lineHeight: 1.4, marginBottom: 3, overflow: "hidden", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical" }}>
                    {book.title}
                </h3>

                {/* Author */}
                <p style={{ color: "var(--c-muted)", fontSize: ".78rem", marginBottom: "auto" }}>
                    by {book.author}
                </p>
          ...
          <span style={{ display: "flex", alignItems: "center", gap: 5, fontSize: ".75rem", fontWeight: 600, color: qc }}>
            {book.available_quantity} {ql}
                    ...
                    <Link href={`/books/${book.id}`} className="btn btn-fill" style={{ padding: "5px 12px", fontSize: ".75rem" }}>
                        Details
                    </Link>
                </div>
            </div>
