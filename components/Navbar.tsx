          ...
          {isPending ? (
            ...
            <>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                {session.user.image
                  ? <img src={session.user.image} style={{ width: 30, height: 30, borderRadius: "50%", objectFit: "cover", border: "2px solid rgba(79,57,246,.45)" }} alt="" />
                ...
                <span style={{ fontSize: ".85rem", color: "var(--c-muted)", maxWidth: 120, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
        ...
        <div className="show-mobile" style={{ borderTop: "1px solid var(--c-border)", background: "var(--c-surface)" }}>
          <div className="wrap" style={{ paddingTop: 12, paddingBottom: 16, display: "flex", flexDirection: "column", gap: 4 }}>
            ...
            <div style={{ marginTop: 8, paddingTop: 8, borderTop: "1px solid var(--c-border)" }}>
              {session?.user ? (
                <>
                  <p style={{ fontSize: ".82rem", color: "var(--c-muted)", padding: "6px 12px" }}>{session.user.name || session.user.email}</p>
